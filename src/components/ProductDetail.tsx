...
// ... (PRODUCTS data remains unchanged)
...
const ProductDetail = () => {
    ...
    const product = PRODUCTS.find(p => p.title.toLowerCase().replace(/ /g, '-') === id) || PRODUCTS[0]; // Fallback for demo
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col">
            ...
            <div className="container mx-auto px-4 py-24 md:py-32 flex-grow">
                <Link to="/services" className="inline-flex items-center text-sm font-bold hover:text-orange-500 mb-8 transition-colors">
                    ← Back to Store
                </Link>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    >
                        <div className="mb-4">
                            <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${product.type === 'course' ? 'bg-orange-100 text-orange-600' : 'bg-purple-100 text-purple-600'}`}>
                                {product.type}
                            </span>
                        </div>
